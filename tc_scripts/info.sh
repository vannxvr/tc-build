...
touch README.md
{
    ...
    echo -e '```\n'
    echo -e "Clone GCC (AArch64) for binutils:\n"
    echo -e '```bash'
    echo -e 'git clone https://github.com/greenforce-project/gcc-arm64 -b main ~/gcc64 --depth=1\n'
    echo -e '```\n'
    echo -e "Clone GCC (Arm32) **If your kernel has a 32-bit vDSO**:\n"
    echo -e '```bash\n'
    echo -e 'git clone https://github.com/greenforce-project/gcc-arm64 -b main ~/gcc32 --depth=1\n'
    echo -e '```\n'
    echo -e 'Make sure you have this toolchain in your `PATH`:\n'
    echo -e '```bash\n'
    echo -e 'export PATH="~/greenforce-clang/bin:~/gcc64/bin:~/gcc32/bin:${PATH}"\n'
    echo -e '```\n'
    ...
    echo -e '- For 32-bit vDSO: `CROSS_COMPILE_ARM32=arm-linux-gnueabi-`\n'
